    ...
    useEffect(() => {
        const fetchData = async () => {
          ...
          `)
                ...
                poData.forEach((po: any) => {
                    // Filter by Vendor first if selected
                    if (selectedVendors.length > 0 && !selectedVendors.includes(po.vendor_id)) {
                        return;
                    }

                    // Find matching items for this PO
                    const poItems: any[] = [];
                    ...
                    poItems.forEach((item, idx) => {
                        rows.push({
                            no: sequence++,
                            id: `${po.id}-${item.id}-${idx}`, // Unique key for React
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button variant="outline" size="icon" onClick={() => {
                        printReport("Laporan Pembelian", data.map(d => ({
                            ...d,
                            user: d.creator_name // Map creator_name to user for unified interface
                        })), 'purchase');
                    }} title="Cetak Laporan">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="w-[120px]">Tanggal Beli</TableHead>
